...
void MapChip::SetNextMapData() {
	...
	{
		// マップチップを初期化
		for (int32_t y = 0; y < data_[z].size(); ++y) {
			for (int32_t x = 0; x < data_[z][y].size(); ++x) {

				mapChip[y][x] = new Model;
				mapChip[y][x]->Initialize();
				mapChip[y][x]->transform.scale = { 0.5f,0.5f,0.5f };
				mapChip[y][x]->transform.translate.x = ((float)x * 1) - (data_[z][y].size() * 0.5f);
				mapChip[y][x]->transform.translate.y = ((float)y * -1) + (data_[z].size() * 0.5f);

				// Map[y][x]が1である場合
				// マップにブロックを配置
				if (data_[z][y][x] == 1) {
					mapChip[y][x]->textureNum = TextureName::STAGETEXTURE;
					mapChip[y][x]->transform.translate.z -= (1 + z);
				}
